...
namespace IS_Team1_Project
{
    public partial class AdminCourses : Form
    {
        ...
        {
            if (txtModule.Text == null)
            {
                return false;
            }
            else if (txtLecturerEmail.Text == null)
            {
                return false;
            }
            else if (txtLecturerNumber.Text == null)
            {
                return false;
            }
            else
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please don't leave any blank textboxes");
            }
        }

        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (sSelected == txtLecturerNumber.Text)
            {
                OleDbConnection conn = new OleDbConnection(@"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + frmLogin.dbPath);
                conn.Open();
            ...
            {
                MessageBox.Show("You can't change a primary key");
            }
        }
